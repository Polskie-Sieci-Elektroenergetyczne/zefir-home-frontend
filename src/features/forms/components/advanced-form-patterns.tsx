...
const advancedSchema = yup.object({
  ...
  team: yup.object({
    name: yup.string().min(2, 'Team name must be at least 2 characters').required(),
    size: yup
      .number()
      .typeError('Team size is required')
  ...
  members: yup
    ...
    .of(
      yup.object({
        name: yup.string().min(1, 'Member name is required').required(),
        role: yup.string().min(1, 'Role is required').required()
      })
    )
    .min(1, 'Add at least one member')
    .required(),
  country: yup.string().min(1, 'Select a country').required(),
  state: yup.string().min(1, 'Select a state').required()
});
...
export default function AdvancedFormPatterns() {
  ...
  const isCheckingUsername = useUsernameAvailability(formik.values.username, formik.setFieldError);
      ...
      <Card>
        <CardHeader>
          ...
          <p className='text-muted-foreground'>
            Demonstrates async validation, linked fields, nested objects, dynamic arrays, and
              ...
              {({ push, remove }) => (
                <div className='space-y-3'>
                  {formik.values.members.map((member, index) => {
                    ...
                    const nameTouched = Boolean(getIn(formik.touched, namePath));
                    const roleTouched = Boolean(getIn(formik.touched, rolePath));

                    const nameError = getIn(formik.errors, namePath) as string | undefined;
                    const roleError = getIn(formik.errors, rolePath) as string | undefined;
                    ...
                    return (
                      <div key={index} className='flex items-start gap-2'>
                        <Field className='flex-1' data-invalid={isNameInvalid || undefined}>
                          <Input
                            ...
                            aria-describedby={
                              isNameInvalid ? `member-name-${index}-error` : undefined
                            }
                          />
                        ...
                        <Field className='flex-1' data-invalid={isRoleInvalid || undefined}>
                          <Input
                            ...
                            aria-describedby={
                              isRoleInvalid ? `member-role-${index}-error` : undefined
                            }
                          />
                  ...
                  {formik.values.members.length > 0 && (
                    <div className='flex flex-wrap gap-1'>
                      {formik.values.members
                        ...
                        .map((member, index) => (
                          <Badge key={`${member.name}-${index}`} variant='secondary'>
                            {member.name}
                  ...
                  {typeof formik.errors.members === 'string' && Boolean(formik.touched.members) && (
                    <FieldError id='members-error' errors={[{ message: formik.errors.members }]} />
                  )}
                </div>
              )}
            </FieldArray>
