...
const demoFormSchema = yup.object({
  ...
  country: yup.string().min(1, 'Please select a country').required(),
  framework: yup.string().min(1, 'Please select a framework').required(),
  interests: yup
    .array()
    .of(yup.string().required())
    .min(1, 'Select at least one interest')
    .required(),
  gender: yup.string().min(1, 'Please select gender').required(),
  newsletter: yup.boolean().required(),
  rating: yup.number().required().min(0).max(10),
  birthDate: yup.date().optional(),
  dateRange: yup.mixed<DateRange>().optional(),
  eventTime: yup.string().optional(),
  favoriteColor: yup.string().optional(),
  otp: yup.string().min(6, 'Please enter 6 digits').required(),
  formatting: yup.array().of(yup.string().required()).optional(),
  tags: yup.array().of(yup.string().required()).min(1, 'Add at least one tag').required(),
  terms: yup.boolean().oneOf([true], 'You must accept the terms').required(),
  avatar: yup.array().of(yup.mixed<File>().required()).optional()
});
 ...
 * check without putting network/business validation into the Yup schema.
 *
        ...
        <Card>
          <CardHeader>
            ...
            <p className='text-muted-foreground'>
              Every possible form input — built with Formik + Yup + shadcn/ui
            </p>
          </CardHeader>
